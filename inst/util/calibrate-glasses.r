...
# with(hcl, plot(x, y, col=colour, pch=20, cex=3 , axes=F, xlab="", ylab=""))


rgb_vals <- expand.grid(b = seq(0, 0.2, length=10), g = 0, r = seq(0.8, 1, length = 10))
rgb_vals$colour <- do.call(rgb, rgb_vals)

with(rgb_vals, plot(r, b, col=colour, pch=20, cex=8, axes=F, xlab="", ylab=""))
with(rgb_vals, text(r, b, cex = 0.5, label = paste(format(r, T, 2), format(g, T, 2), format(b, T, 2))))
