...
test_that("radial_tour mvar must index a valid variable", {
  # radial_tour() does not validate mvar at construction time — the bad index
  # only surfaces when the tour is stepped. We therefore drive it through
  # new_tour() + one step to trigger the error.
  start <- basis_random(6, 2)

  # A valid mvar (boundary case) should work fine
  expect_no_error({
    tour <- new_tour(dat, radial_tour(start, mvar = 6))
    tour(0.05)
  })

  # Out-of-range mvar should error when the tour is first stepped
  expect_error({
    tour <- new_tour(dat, radial_tour(start, mvar = 0))
    tour(0.05)
  }, label = "mvar = 0 errors when tour is stepped")

  expect_error({
    tour <- new_tour(dat, radial_tour(start, mvar = 7))
    tour(0.05)
  }, label = "mvar = 7 errors when tour is stepped (p = 6)")
})
