...
The second argument we used when calling `animate` limits the number of frames generated for the purpose of this vignette. In practice, when running a tour live, you can omitt the `max_frames` argument and watch the stream of new bases as they are generated.
...
In this case we do not need to specify the `max_frames` argument, because the tour will automatically stop once it has found the best view.
