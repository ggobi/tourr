...
p2 <- kol_geodesic_polish %>% 
  ...
  geom_line() + 
  geom_hline(yintercept = theo_kol, alpha = 0.5) + 
  ggtitle("kol index, search_geodesic with polish") + 
  ylim(c(0, 15))
...
p4 <- kol_better_polish %>% 
  ...
  geom_line() + 
  geom_hline(yintercept = theo_kol, alpha = 0.5) + 
  ggtitle("kol index, search_better with polish") + 
  ylim(c(0, 15))
...
p6 <- kol_cdf_better_polish %>% 
  ...
  geom_line() + 
  geom_hline(yintercept = theo_kol_cdf, alpha = 0.5) + 
  ggtitle("kol_cdf index, search_better with polish") + 
  ylim(c(0, 0.22))
...
p2 <- kol_2var_better_random %>% 
  filter(info == "interpolation") %>%
  mutate(id = row_number())  %>%
  ggplot(aes(x = id, y = index_val)) + geom_line() + geom_point(aes(col = as.factor(tries))) + 
  ggtitle("kol index, better_random, no polish")

p2_end <- kol_2var_better_random %>% group_by(tries) %>% mutate(id = row_number()) %>% 
  filter(id == max(id)) %>% ungroup() %>% filter(tries < max(tries)) %>% mutate(id = row_number()) %>% 
  ggplot(aes(x = id, y = index_val)) + 
  geom_point(aes(col = as.factor(tries))) + 
  geom_line() + 
...
# what has been found is x2
kol_2var_better %>% arrange(-index_val) %>% head(1) %>% pull(basis) %>% .[[1]]
kol_2var_better_random %>% arrange(-index_val) %>% head(1) %>% pull(basis) %>% .[[1]]
```
...
kol_2var_better %>% filter(info == "interpolation") %>% view
kol_2var_cdf_better_random_polish %>% filter(info == "interpolation", id == 65) %>% pull(basis) 


kol_2var_cdf_better_random_polish %>% filter(info != "polish") %>% 
  mutate(id = row_number())  %>%
  ggplot(aes(x = id, y = index_val)) + geom_line() + geom_point(aes(col = info))


kol_2var_cdf_better_polish %>% filter(info == "interpolation") %>% pull(basis) %>% tail(1) %>% .[[1]]
kol_2var_cdf_better_polish %>% filter(info == "polish_best") %>% pull(basis) %>% tail(1) %>% .[[1]]


devtools::load_all()
set.seed(123456)
kol_2var_better_polish <- animate_dist(data_mult[,c(1,2,7:10)], 
                                                tour_path = guided_tour(kol(), d = 1,
                                                                        search_f = search_better, 
                                                                        max.tries = 1000),
                                       rescale = FALSE, verbose = TRUE, polish = FALSE)

devtools::load_all()
set.seed(123456)
kol_2var_better_random_polish <- animate_dist(data_mult[,c(1,2,7:10)], 
                                                tour_path = guided_tour(kol(), d = 1,
                                                                        search_f = search_better_random, 
                                                                        max.tries = 1000),
                                       rescale = FALSE, verbose = TRUE, polish = FALSE)

calc_kol(matrix(c(0, 1, 0, 0, 0, 0), nrow = 6), data = data_mult[,c(1,2,7:10)])
calc_kol(matrix(c(0, 0, 1, 0, 0, 0), nrow = 6), data = data_mult[,c(1,2,7:10)])
calc_kol(matrix(c(-0.028, 0.991, 0.097, -0.037, -0.014, 0.079), nrow = 6), data = data_mult[,c(1,2,7:10)])
